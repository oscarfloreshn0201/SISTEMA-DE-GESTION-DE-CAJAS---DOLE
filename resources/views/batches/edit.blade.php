...
<div class="bg-white rounded-lg shadow p-6">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold">Editar Batch</h2>
            <p class="text-gray-600">Pertenece a la caja: <strong>{{ $batch->caja->display_name }}</strong></p>
        </div>
        <a href="{{ route('cajas.show', $batch->caja) }}" class="text-gray-500 hover:text-gray-700">← Volver</a>
    </div>
    ...
    <form method="POST" action="{{ route('batches.update', $batch) }}">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="numero_batch" value="{{ old('numero_batch', $batch->numero_batch) }}" 
        ...
        <div class="mb-4">
            ...
            <input type="text" name="folder" value="{{ old('folder', $batch->folder) }}" 
                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500"
                   required>
            @error('folder')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    ...
    @enderror
        ...
        <div class="mb-6">
            ...
            <textarea name="descripcion" rows="3" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-500">{{ old('descripcion', $batch->descripcion) }}</textarea>
        </div>
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('cajas.show', $batch->caja) }}" class="bg-gray-300 text-gray-700 px-6 py-2 rounded-lg hover:bg-gray-400">
                Cancelar
            </a>
        </div>
    </form>
